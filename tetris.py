...
#MIT COURSE 6.189 FINAL PROJECT
...
class Block(Rectangle):
    ...
    def can_move(self, board, dx, dy):
        ...
        #if (self.x+dx) > 0 and (self.x+dx) < board.width and (self.y+dy) > 0 and (self.y+dy) < board.height:
        return board.can_move(dx+self.x,dy+self.y)
...
class Shape():
    ...
    def can_move(self, board, dx, dy):
        ...
        for blockobj in self.blocks:
            if not blockobj.can_move(board,dx,dy):
                return False
        return True
    ...
    def can_rotate(self, board):
        ...
        rotationdir = self.get_rotation_dir()
        center = self.center_block
        for block in self.blocks:
            x = center.x - rotationdir*center.y + rotationdir*block.y
            y = center.y + rotationdir*center.x - rotationdir*block.x
            newx = x - block.x
            newy = y - block.y
            if not block.can_move(board, newx, newy):
                return False
        return True
...
class Board():
    ...
    def add_shape(self, shape):
        ...
        for blockobj in shape.get_blocks():
            self.grid[blockobj.x,blockobj.y] = blockobj
    ...
    def delete_row(self, y):
        ...
        for xval in range(0,self.width):
            self.grid[xval, y].undraw()
            del self.grid[xval, y]
    ...
    def move_down_rows(self, y_start):
        ...
        for yval in reversed(range(0,y_start+1)):
            for xval in range(0,self.width):
                if (xval,yval) in self.grid:
                    blockobj = self.grid[xval,yval]
                    blockobj.undraw()
                    blockobj.move(0,1)
                    blockobj.draw(self.canvas)
                    del self.grid[xval,yval]
                    self.grid[xval,yval+1] = blockobj
    ...
    def remove_complete_rows(self):
        ...
        '''
        addpoints = 0
        for yval in range(0, self.height):
            if self.is_row_complete(yval):
                addpoints += 1
                self.delete_row(yval)
                self.move_down_rows(yval-1)
        return addpoints
    ...
    def game_over(self):
        ...
        textobj = Text(Point(150,200),"GAME OVER")
        textobj.setSize(40)
        textobj.draw(self.canvas)
...
class Tetris():
    ...
    def __init__(self, win):
        ...
        self.scoreboard = Scorecounter(self.board)
    ...
    def create_new_shape(self):
        ...
        return random.choice(self.SHAPES)(Point(x,0))
    ...
    def do_move(self, direction):
        ...
        xmove, ymove = self.DIRECTION[direction]
        if  (self.current_shape.can_move(self.board,xmove,ymove)):
            self.current_shape.move(xmove,ymove)
            return True
        else:
            print direction
            if direction == 'Down':
                self.board.add_shape(self.current_shape)
                self.scoreboard.pointtotal += self.board.remove_complete_rows()
...
class Scorecounter():
    ...
    def __init__(self,board):
        self.pointtotal = 0
        self.scoretext = Text(Point(100,25),"[p to pause] Score is " + str(self.pointtotal))
        self.scoretext.draw(board.canvas)
        print(self.pointtotal)

    def update_score(self):
        self.scoretext.setText("[p to pause] Score is " + str(self.pointtotal))
